...
# =============================================================================
# LOAD SECTION 6 ARTIFACTS (WITH PATCH FOR LOGISTICREGRESSION)
# =============================================================================

@st.cache_resource
def load_section6_artifacts():
    """Load all artifacts saved from Section 6 and patch cross_model for
    ...
    # Patch cross_model for sklearn version mismatch
    model = cross_artifacts.get("model", None)
    try:
        if model is not None:
            # Older pickles may expect attributes that don't exist in current sklearn.
            if not hasattr(model, "multi_class"):
                model.multi_class = "auto"
            if not hasattr(model, "n_features_in_") and hasattr(model, "coef_"):
                model.n_features_in_ = model.coef_.shape[1]
    except Exception:
        # If patching fails, we'll still be safe in safe_cross_proba
        pass
...
def build_model_frame_from_canonical(df_canon: pd.DataFrame,
    ...
    Any feature not provided by the user is left as NaN and imputed by
    ...
    # All-NaN frame with the same number of rows as df_canon
    model_df = pd.DataFrame(columns=expected_cols)
    model_df.loc[0:len(df_canon) - 1] = np.nan

    # Explicit mapping from canonical names to model feature names
    canon_to_model = {
        "age": "Age",
        "gender": "Gender",
...
def safe_cross_proba(cross_model, X_od_new: np.ndarray) -> np.ndarray:
    ...
    If the LogisticRegression object is from an older sklearn and missing
    attributes, we patch them. If anything still fails, return 0.5.
    """
    try:
        if not hasattr(cross_model, "multi_class"):
            cross_model.multi_class = "auto"
        if not hasattr(cross_model, "n_features_in_") and hasattr(cross_model, "coef_"):
            cross_model.n_features_in_ = cross_model.coef_.shape[1]
